...
async def user_profile(msg: Message):
    ...
    user_data = (
        session.query(User.user_name, User.user_phone, User.user_tg_id)
        .filter(User.user_tg_id == msg.from_user.id)
        .first()
    )
    ...
    if user_data:
        ...
        await msg.answer(text="<b>Профиль не найден</b> 🤔", parse_mode="HTML")
